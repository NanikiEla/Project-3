...
@csrf_exempt
def productImg_by_id(request, productId):
    try:
...
def delete_file(request, filename, img):
    if request.method == 'GET':
...
def filter_range_price(request, minPrice, maxPrice, price__range):
    try:
...
def filter_min_price(request, minPrice):
    try:
        products = Product.objects.filter(price__get=minPrice)
    except:
        return HttpResponse(status=404)
...
def filter_max_price(request, maxPrice):
    try:
        products = Product.objects.filter(price__get=maxPrice)
    except:
        return HttpResponse(status=404)
...
def filter_rating(request, rating):
    try:
        products = Product.objects.filter(rating__get=rating)
    except:
        return HttpResponse(status=404)
...
def filter_price_and_rating(request, minPrice, maxPrice, price__range, rating):
    try:
        products = Product.objects.filter(price__range(minPrice, maxPrice)).filter(rating_get=rating)
    except:
        return HttpResponse(status=404)
...
def filter_price_and_condition(request, minPrice, maxPrice, price__range, condition):
    try:
...
def filter_rating_and_condition(request, rating, condition):
    try:
        products = Product.objects.filter(rating_get=rating).filter(condition=condition)
    except:
        return HttpResponse(status=404)
...
def filter_all(request, minPrice, maxPrice, price__range, rating, condition):
    try:
        products = Product.objects.filter(price__range(minPrice, maxPrice)).filter(rating_get=rating).filter(condition=condition)
    except:
        return HttpResponse(status=404)
...
def get_cart_item_by_cart_id(request, cartId, cartItem):
    try:
        cartId = cartId.objects.filter(cartId_id=cartId).prefetch_related('productId').order_by('create_at')
    except:
        return HttpResponse(status=404)
